...
def pose_ros_enu_to_airsim_ned(pos, quat):
    ...
    R_ros_enu_to_as_ned = np.asarray([[0, 1, 0], [1, 0, 0], [0, 0, -1]])
...
def save_anno_path(env_id, path):
    anno_curve_path = paths.get_anno_curve_path(env_id)
    path_json = {"x_array": list(path[:, 0]), "z_array": list(path[:, 1])}
...
def load_env_img(
    ...
    img = imread(
        paths.get_env_image_path(
            env_id, real_drone=real_drone, dir_override=directory_override
        )
    )
...
def get_landmark_locations_airsim(config_json=None, add_empty=False, env_id=None):
    assert (
        config_json is not None or env_id is not None
    ), "At least one of config_json or env_id must be provided"
