...
from ros2controller_async import SimpleMavlinkController
...
class LMP_interface:
    def __init__(
        self,
        env: Union[VoxPoserROS2DroneEnv, DummyEnv],
        lmp_config,
        controller_config,
        planner_config,
        ...
        elif _controller_type == 'SimpleMavlinkController':
            self._controller = SimpleMavlinkController(self._env, controller_config)
        else:
    ...
    def detect(self, obj_name, enable_memory=False):
    ...
    def execute(
        ...
        if dynamic:
            if _async:
                return self.execute_quad_dynamic_async(
                    movable_obs_func,
                    affordance_map,
                    avoidance_map,
                    velocity_map,
            ...
            return self.execute_quad_dynamic(
                movable_obs_func,
                affordance_map,
                avoidance_map,
                velocity_map,
    ...
    def _post_process_detect_result(self,obj_name, obs_results):
        object_obs_list = []
    ...
    def detect_quad(self, obj_name, enable_memory=False):
        ...
        if obj_name.lower() in EE_ALIAS:
            ...
            if (
                ...
                if enable_memory and (obj_name in self._latest_obs_results):
                    warnings.warn(
                        f"[interface.py:detect_quad] failed to detect {obj_name}, using memory instead"
                    )
                    obs_results = self._latest_obs_results[
                        obj_name
                    ]  # use memory my cause some problems, as the object may have moved
                else:
                    return object_obs_list
            elif enable_memory:
                self._latest_obs_results.update({obj_name: obs_results})
            object_obs_list = self._post_process_detect_result(obj_name=obj_name, obs_results=obs_results)
        return object_obs_list
    ...
    def _plan(
        ...
        while not self._stop_planing.is_set():  # TODO self._cfg["max_plan_iter"]
            ...
            print(
                f"{bcolors.OKBLUE}[interfaces.py | _plan | {get_clock_time()} | iter_{plan_iter}] planner time: {time.time() - start_time:.3f}s{bcolors.ENDC}"
            )
            plan_iter += 1
            ...
            print(
                f"{bcolors.OKBLUE}[interfaces.py | _plan | {get_clock_time()}] weak up, waiting for lock{bcolors.ENDC}"
            )
        print(
            f"{bcolors.OKBLUE}[interfaces.py | _plan | {get_clock_time()}] target readch, finished planning{bcolors.ENDC}"
        )
    ...
    async def _plan_once(
        ...
        print(
            f"{bcolors.OKBLUE}[interfaces.py | _plan | {get_clock_time()} | iter_{self.plan_iter}] start planning{bcolors.ENDC}"
        )
        movable_obs = movable_obs_func()
        _affordance_map = affordance_map()
        _pre_avoidance_map = avoidance_map()
        ...
        print(
            f"{bcolors.OKBLUE}[interfaces.py | _plan | {get_clock_time()} | iter_{self.plan_iter}] planner time: {time.time() - start_time:.3f}s{bcolors.ENDC}"
        )
        self.plan_iter += 1
        assert len(path_voxel) > 0, "path_voxel is empty"
        # convert voxel path to world trajectory, and include rotation, velocity, and gripper information
        ...
        if not debug_traj_confirm:
            ...
            print(
                f"{bcolors.OKBLUE}[interfaces.py | _plan | {get_clock_time()} | iter_{self.plan_iter}] traj auto comfirmated!{bcolors.ENDC}"
            )
        print(
            f"{bcolors.OKBLUE}[interfaces.py | _plan | {get_clock_time()} | iter_{self.plan_iter}] output traj!{bcolors.ENDC}"
        )
        return self.traj_world
    ...
    def _execute_wp_by_wp(self, movable_obs_func, affordance_map_func, _plan_cond):
        """
        # get traj from plan thread,
        # execute one point -> check traj terminated -> check task terminated -> if not (continue traj or replan), if yes finished
        """
        # execute path
        step_index = 0
        while True:
            ...
            controller_info = self._controller.execute(
                movable_obs=movable_obs_func(), target=waypoint
            )  # TODO check if the movement is finished as controller_info["mp_info"] not used
            ...
            # execute results judgement, loggging
            if controller_info["mp_info"] == -1:
                print(
                    f'{bcolors.OKBLUE}[interfaces.py | {get_clock_time()}] failed waypoint {step_index} \
                        (wp: {waypoint[0].round(3)}, actual: {movable_obs["_position_world"].round(3)}, target: {self.traj_world[-1][0].round(3)}, \
                            start: {self.traj_world[0][0].round(3)}, dist2target: {dist2target.round(3)}); mp info: {controller_info["mp_info"]}{bcolors.ENDC}'
                )
            else:
                print(
                    f'{bcolors.OKBLUE}[interfaces.py | {get_clock_time()}] completed waypoint {step_index} \
                        (wp: {waypoint[0].round(3)}, actual: {movable_obs["_position_world"].round(3)}, target: {self.traj_world[-1][0].round(3)}, \
                            start: {self.traj_world[0][0].round(3)}, dist2target: {dist2target.round(3)}){bcolors.ENDC}'
                )
            # reaching judgement, reach target -> clear all remained waypoints, not reach target -> pop the first waypoint
            if (
                dist2target <= 0.1
            ):  # check for current trajectory, if the distance to the final target is less than 0.1
                print(
                    f"{bcolors.OKBLUE}[interfaces.py | {get_clock_time()}] close to the target point of this traj; remove all remained waypoints{bcolors.ENDC}"
                )
                self.traj_world.clear()
            else:
                self.traj_world.pop(0)
            # check if replan is needed, details in _check_terminate
            if self.traj_world == []:
                print(
    ...
    def execute_quad_dynamic_async(
        self,
        movable_obs_func: Callable,
        affordance_map: Callable = None,
        avoidance_map: Callable = None,
        ...
        if affordance_map is None:
            print(
                f"{bcolors.OKBLUE}[interfaces.py | {get_clock_time()}] affordance_map is None{bcolors.ENDC}"
            )
            return

        object_centric = (
            movable_obs_func()["name"] not in EE_ALIAS
        )  # false as the object is the quadricopter
        assert not object_centric, "the movable object must be the quadricopter"
        
        plan_task = loop.create_task(self._plan_once(
            movable_obs_func,
            affordance_map,
            avoidance_map,
            velocity_map,
        ...
        while not task_finished:
            execute_task = loop.create_task(self._controller.execute(
                movable_obs=None, target=traj_world
            ))
            
            print(
                f"{bcolors.OKBLUE}[interfaces.py | {get_clock_time()}] start executing path via controller, timeout: {1/plan_hz}{bcolors.ENDC}"
            )
            ...
            if execute_task in done:
                ...
                print(
                    f"{bcolors.OKBLUE}[interfaces.py | {get_clock_time()}] replanning{bcolors.ENDC}"
                )
                plan_task = loop.create_task(self._plan_once(
                    movable_obs_func,
                    affordance_map,
                ...
                if execute_task in done:
                    task_finished = True
                    print(
                        f"{bcolors.OKBLUE}[interfaces.py | {get_clock_time()}] finished executing path via controller after replanning{bcolors.ENDC}"
                    )
                    break
                else:
                    print(
                        f"{bcolors.OKBLUE}[interfaces.py | {get_clock_time()}] canceling the previous mission{bcolors.ENDC}"
                    )
                    # 发送新的 traj_world
                    loop.run_until_complete(self._controller.cancel_mission())
                    done, pending = loop.run_until_complete(asyncio.wait(
                        [execute_task],return_when=asyncio.FIRST_COMPLETED
                    ))
                    print(
                        f"{bcolors.OKBLUE}[interfaces.py | {get_clock_time()}] canceled the previous mission{bcolors.ENDC}"
                    )
    ...
    def execute_quad_dynamic(
        ...
        if affordance_map is not None:
            ...
            with _plan_cond:
                if len(self.traj_world) == 0:
                    print(
                        f"{bcolors.OKBLUE}[interfaces.py | {get_clock_time()}] waiting for path to be planned{bcolors.ENDC}"
                    )
                    _plan_cond.wait()
            print(
                f"{bcolors.OKBLUE}[interfaces.py | {get_clock_time()}] start executing path via controller){bcolors.ENDC}"
            )
            low_level_execute_func(movable_obs_func, affordance_map, _plan_cond)
        print(
            f"{bcolors.OKBLUE}[interfaces.py | {get_clock_time()}] finished executing path via controller{bcolors.ENDC}"
        )
        self._env.visualizer.save_gifs()
...
def setup_LMP(env, general_config, debug=False, engine_call_fn=None, log_dir="./"):
    ...
    variable_vars["execute"] = partial(
        lmp_env.execute, dynamic=lmp_env_config["execute_dynamic"]
    )
