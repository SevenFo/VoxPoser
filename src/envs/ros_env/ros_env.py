...
class VoxPoserROSDroneEnv:
    ...
    def init_task(self):
        ...
        rospy.loginfo(
            f"reset env: {rospy.ServiceProxy('/airsim_node/reset', SetBool)(SetBool._request_class(True)).message}"
        )
    ...
    def init_ros(self):
        ...
        self._drone_base_link_frame_id = rospy.get_param(
            "/airsim_node/drone_base_link_frame_id"
        )
        airsim_client_host = rospy.get_param(
            "/airsim_node/airsim_client_host", "localhost"
        )
        airsim_client_port = rospy.get_param("/airsim_node/airsim_client_port", 10000)
        update_airsim_img_response_every_n_sec = rospy.get_param(
            "/airsim_node/update_airsim_img_response_every_n_sec", 1
        )

        self._cmd_pub = rospy.Publisher(
            "/airsim_node/vel_cmd_world_frame", Twist, queue_size=1
        )
        self._point_front_center_sub = rospy.Subscriber(
            "/airsim_node/drone_1/front_center/1/registered/points_transformed",
            PointCloud2,
            self.point_front_center_sub_callback,
        )
        self._odom_sub = rospy.Subscriber(
            "/airsim_node/drone_1/odom_local_enu", Odometry, self.odom_sub_callback
        )

        self.rgb_image_front_center_sub = rospy.Subscriber(
            "/airsim_node/drone_1/front_center/0/image_raw",
            Image,
            self.rgb_image_sub_front_center_callback,
        )
        self.cloudpoint_front_center_sub = rospy.Subscriber(
            "/airsim_node/drone_1/front_center/1/registered/points_transformed",
            PointCloud2,
            self.cloudpoint_sub_front_center_callback,
        )
        self.camera_info_front_center_sub = rospy.Subscriber(
            "/airsim_node/drone_1/front_center/1/camera_info",
            CameraInfo,
            self.camera_info_sub_front_center_callback,
        )
        self.rgb_image_front_left_sub = rospy.Subscriber(
            "/airsim_node/drone_1/front_left/0/image_raw",
            Image,
            self.rgb_image_sub_front_left_callback,
        )
        self.cloudpoint_front_left_sub = rospy.Subscriber(
            "/airsim_node/drone_1/front_left/1/registered/points_transformed",
            PointCloud2,
            self.cloudpoint_sub_front_left_callback,
        )
        self.camera_info_front_left_sub = rospy.Subscriber(
            "/airsim_node/drone_1/front_left/1/camera_info",
            CameraInfo,
            self.camera_info_sub_front_left_callback,
        )
        self.rgb_image_front_right_sub = rospy.Subscriber(
            "/airsim_node/drone_1/front_right/0/image_raw",
            Image,
            self.rgb_image_sub_front_right_callback,
        )
        self.cloudpoint_front_right_sub = rospy.Subscriber(
            "/airsim_node/drone_1/front_right/1/registered/points_transformed",
            PointCloud2,
            self.cloudpoint_sub_front_right_callback,
        )
        self.camera_info_front_right_sub = rospy.Subscriber(
            "/airsim_node/drone_1/front_right/1/camera_info",
            CameraInfo,
            self.camera_info_sub_front_right_callback,
        )
        self.rgb_image_down_center_sub = rospy.Subscriber(
            "/airsim_node/drone_1/down_center/0/image_raw",
            Image,
            self.rgb_image_sub_down_center_callback,
        )
        self.cloudpoint_down_center_sub = rospy.Subscriber(
            "/airsim_node/drone_1/down_center/1/registered/points_transformed",
            PointCloud2,
            self.cloudpoint_sub_down_center_callback,
        )
        self.camera_info_down_center_sub = rospy.Subscriber(
            "/airsim_node/drone_1/down_center/1/camera_info",
            CameraInfo,
            self.camera_info_sub_down_center_callback,
        )
        self.rgb_image_rear_center_sub = rospy.Subscriber(
            "/airsim_node/drone_1/rear_center/0/image_raw",
            Image,
            self.rgb_image_sub_rear_center_callback,
        )
        self.cloudpoint_rear_center_sub = rospy.Subscriber(
            "/airsim_node/drone_1/rear_center/1/registered/points_transformed",
            PointCloud2,
            self.cloudpoint_sub_rear_center_callback,
        )
        self.camera_info_rear_center_sub = rospy.Subscriber(
            "/airsim_node/drone_1/rear_center/1/camera_info",
            CameraInfo,
            self.camera_info_sub_rear_center_callback,
        )
        rospy.init_node("voxposer")

        self._action_client = actionlib.SimpleActionClient(
            "/airsim_node/goto", MoveBaseAction
        )

    def point_front_center_sub_callback(self, msg):
        point = np.array(
            list(pl2.read_points(msg, field_names=("x", "y", "z"), skip_nans=True))
        )
        # print(point.shape)
    ...
    def _get_rgb_frames(self):
        ...
        for cam in self.camera_names:
            while f"{cam}_rgb" not in self.latest_obs:
                ...
                rospy.sleep(0.1)
            rgb_frames[cam] = self.latest_obs[f"{cam}_rgb"].transpose([2, 0, 1])
    ...
    def cloudpoint_sub_front_left_callback(self, msg: PointCloud2):
        ...
        # print(point.shape)
        self.latest_obs.update({"front_left_cloudpoint": point})
    ...
    def cloudpoint_sub_front_right_callback(self, msg: PointCloud2):
        ...
        # print(point.shape)
        self.latest_obs.update({"front_right_cloudpoint": point})
    ...
    def cloudpoint_sub_down_center_callback(self, msg: PointCloud2):
        ...
        # print(point.shape)
        self.latest_obs.update({"down_center_cloudpoint": point})
    ...
    def cloudpoint_sub_rear_center_callback(self, msg: PointCloud2):
        ...
        # print(point.shape)
        self.latest_obs.update({"rear_center_cloudpoint": point})
