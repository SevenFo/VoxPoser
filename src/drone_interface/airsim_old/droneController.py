...
from data_io.drif.env import load_env_config
from data_io.drif.helpers import save_json
from data_io.drif.units import DEPTH_SCALE
import data_io.drif.paths as paths
from drone_interface.droneControllerBase import DroneControllerBase
from drone_interface.airsim_old.airsimClient import *
from drone_interface.airsim_old.rate import Rate
from drone_interface.exceptions import AirSimConnectionError
...
def startAirSim(controller, instance_id, port):
    try:
        ...
        try:
            print(e)
            print(
                "Failed to connect to client on port "
                + str(port)
                + "! Starting new AirSim..."
            )
            if P.get_current_parameters()["Environment"].get("headless"):
...
def _write_airsim_settings(self):
    ...
    airsim_settings_path = P.get_current_parameters()["Environment"][
...
class DroneController(DroneControllerBase):
    def __init__(self, instance=0, flight_height=100.0, **kwargs):
        """_summary_

        Args:
            instance (int, optional): _description_. Defaults to 0.
            flight_height (float, optional): _description_. Defaults to 100.0.
            kwargs:
                start_height_offset (float, optional): _description_. Defaults to 0.0.
                clock_speed (float, optional): the clock_speed param of AirSim. Defaults to 1.0.
        """
        super(DroneController, self).__init__()
        self.instance = instance
        self.port = PORTS[instance]
        self.clock_speed = (
            kwargs["clock_speed"] if "clock_speed" in kwargs["clock_speed"] else 1.0
        )
        ...
        self.height_offset = (
            kwargs["start_height_offset"]
            if "start_height_offset" in kwargs["start_height_offset"]
            else 0.0
        )
    ...
    def _start_call(self):
        try:
            ...
            print_error("Error encountered during connecting to airsim server!")
            print_error(e)
            ...
            raise AirSimConnectionError(
                "Exception encountered during start_call, please restart airsim"
            )
    ...
    def send_local_velocity_command(self, cmd_vel):
        """
        :param cmd_vel: x in m/s and yaw rate in rad/s, the second value (vel_y) is ignored
        :return:
        """
        try:
            ...
            if len(cmd_vel) == 3:
                ...
                tgt_h = self.flight_height - self.height_offset
                self.client.moveByVelocityZ(
                    vel_x, vel_y, tgt_h, self.control_interval, yaw_mode=yaw_mode
                )
        except msgpackrpc.error.TimeoutError as e:
            # startAirSim(self, self.instance, self.port)
            raise AirSimConnectionError("Simulator Died, please restart airsim")
    ...
    def teleport_to(self, position, yaw):
        ...
        try:
            ...
            for i in range(10):
                ...
                if (
                    np.linalg.norm(pos_as - curr_pos_np) < 1.0
                    and math.fabs(tgt_h - curr_pos.z_val) < 1.0
                ):
                    teleported = True
                    break
            if not teleported:
    ...
    def get_state(self, depth=False, segmentation=False):
        try:
            ...
            cam_pos = [
                pos_dict[b"x_val"],
                pos_dict[b"y_val"],
                pos_dict[b"z_val"] + self.height_offset,
            ]
            cam_rot = [
                rot_dict[b"w_val"],
                rot_dict[b"x_val"],
            ...
            img_state = {"rgb": image_raw, "depth": depth_raw, "seg": seg_raw}
            ...
            # Convert to numpys
            drn_pos = np.asarray(
                [pos.x_val, pos.y_val, pos.z_val + self.height_offset]
            )  # xyz
            ...
            drone_state = np.concatenate(
                (drn_pos, drn_rot, drn_vel, cam_pos, cam_rot), axis=0
            )
