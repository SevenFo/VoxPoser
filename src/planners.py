...
class PathPlanner:
    ...
    def optimize(
        ...
        obstacle_map = gaussian_filter(
            obstacle_map, sigma=self.config.obstacle_map_gaussian_sigma
        )
        obstacle_map = normalize_map(obstacle_map)
        # combine target_map and obstacle_map
        costmap = (
            target_map * self.config.target_map_weight
            + obstacle_map * self.config.obstacle_map_weight
        )
        ...
        # optimize
        print(
            f"[planners.py | {get_clock_time(milliseconds=True)}] start optimizing, start_pos: {start_pos}"
        )
        for i in range(self.config.max_steps):
            # calculate all nearby voxels around current position
            all_nearby_voxels = self._calculate_nearby_voxel(
                current_pos, object_centric=object_centric
            )
            # calculate the score of all nearby voxels
            nearby_score = _costmap[
                all_nearby_voxels[:, 0],
                all_nearby_voxels[:, 1],
                all_nearby_voxels[:, 2],
            ]
            ...
            _costmap[
                current_pos[0].round().astype(int),
                current_pos[1].round().astype(int),
                current_pos[2].round().astype(int),
            ] += 1
        ...
        print(
            f"[planners.py | {get_clock_time(milliseconds=True)}] optimization finished; path length: {len(raw_path)}"
        )
        # postprocess path
    ...
    def _get_stop_criteria(self):
        def no_nearby_equal_criteria(current_pos, costmap, stop_threshold):
            ...
            current_pos_discrete = (
                current_pos.round().clip(0, self.map_size - 1).astype(int)
            )
            current_cost = costmap[
                current_pos_discrete[0],
                current_pos_discrete[1],
                current_pos_discrete[2],
            ]
            nearby_locs = self._calculate_nearby_voxel(
                current_pos, object_centric=False
            )
            nearby_equal = np.any(
                costmap[nearby_locs[:, 0], nearby_locs[:, 1], nearby_locs[:, 2]]
    ...
    def _calculate_nearby_voxel(self, current_pos, object_centric=False):
        ...
        if object_centric:
            ...
            offsets_grid = np.array(np.meshgrid(offsets, offsets, offsets)).T.reshape(
                -1, 3
            )
            # Remove the [0, 0, 0] offset, which corresponds to the current position
            offsets_grid = offsets_grid[np.any(offsets_grid != [0, 0, 0], axis=1)]
    ...
    def _postprocess_path(self, path, raw_target_map, object_centric=False):
        ...
        if len(path_trimmed) > 1:
            ...
            if length > target_spacing:
                curr_spacing = np.linalg.norm(
                    path_trimmed[1:] - path_trimmed[:-1], axis=1
                ).mean()
        ...
        if raw_target_map[last_waypoint[0], last_waypoint[1], last_waypoint[2]] == 0:
            ...
            closest_target_idx = np.argmin(
                np.linalg.norm(target_pos - last_waypoint, axis=1)
            )
